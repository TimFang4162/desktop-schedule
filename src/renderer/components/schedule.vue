...
<template>
  ...
  >
    ...
    <v-list-item>
      <v-list-item-content class="pt-1">
        <v-card-title class="text-h6 pa-0">
          ...
          <v-menu v-if="!onEdit" transition="slide-y-transition" bottom>
            ...
            <v-card>
              ...
              <settings
                v-if="settingsDialog" :settings-dialog="settingsDialog" :config="config"
                @close="closeSettings()"
              />
              <about v-if="aboutDialog" :about-dialog="aboutDialog" @close="closeAbout()" />
            </v-card>
          </v-menu>
        ...
        <v-card-title class="pa-0">
          <v-tooltip bottom>
            <template #activator="{ on, attrs }">
              <v-btn small class="pl-1 pr-1" v-bind="attrs" text v-on="on" @click="nowDate = new Date()">
          ...
          <v-tooltip bottom>
            <template #activator="{ on, attrs }">
              <v-btn
                icon small v-bind="attrs" v-on="on"
    ...
    <v-list nav>
      <v-dialog v-if="editDialog" v-model="editDialog" persistent max-width="35%">
        <v-card>
          <v-card-title>
            ...
            <v-row align="center">
              ...
              <v-col cols="6">
                <v-select
                  ...
                  :items="Object.entries(config.courses)" item-text="[1].name" item-value="[0]" label="Select"
                  single-line
                />
              </v-col>
              <v-col cols="6">
              ...
              <v-col cols="6">
                <v-menu
                  ref="menu" v-model="editStartTimeMenu" :close-on-content-click="false"
                  transition="scale-transition" offset-y
                >
                  <template #activator="{ on, attrs }">
                    <v-text-field
                      v-model="tempConfig.schedule[scheduleOfTheDay][onEditId].time_start"
                      label="点击打开时间选择器" prepend-icon="mdi-clock-time-four-outline" readonly v-bind="attrs"
                      v-on="on"
                    />
                  </template>
                  <v-time-picker
                    v-if="editStartTimeMenu"
                    v-model="tempConfig.schedule[scheduleOfTheDay][onEditId].time_start" format="24hr"
                  />
                </v-menu>
              </v-col>
              <v-col cols="6">
              ...
              <v-col cols="6">
                <v-menu
                  ref="menu" v-model="editEndTimeMenu" :close-on-content-click="false" transition="scale-transition"
                  offset-y
                >
                  <template #activator="{ on, attrs }">
                    <v-text-field
                  ...
                  <v-time-picker
                    v-if="editEndTimeMenu" v-model="tempConfig.schedule[scheduleOfTheDay][onEditId].time_end"
                    format="24hr"
                  />
                </v-menu>
              </v-col>
          ...
          <v-card-actions>
            <v-spacer />
            <v-btn color="darken-1" text @click="editDialog = false">
              取消
            </v-btn>
            <v-btn color="darken-1" text @click="courseSaveEdit()">
              保存
            </v-btn>
          </v-card-actions>
        </v-card>
      ...
      <template v-for="(each, index) in config.schedule[scheduleOfTheDay]">
        <v-card
          ...
          elevation="0" ripple @click="courseOnClick(config.courses[each.ref])"
        >
      ...
      <v-card v-if="onEdit" class="bg-t mb-2" elevation="0" ripple @click="addCourse()">
        <v-card-title class="text-h5 pb-2">
